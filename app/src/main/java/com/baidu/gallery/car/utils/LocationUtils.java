...
import com.baidu.gallery.car.config.BaiduConfig;
import com.baidu.gallery.car.geocoding.GeocodingFactory;
...
public class LocationUtils {
    ...
    public static String getLocationFromImage(Context context, String imageUrl) {
        ...
        try {
            Log.d(TAG, GPS_DEBUG + "========== 开始从图片提取GPS信息 ==========");
            Log.d(TAG, GPS_DEBUG + "图片URL: " + imageUrl);
            
            // 从URL下载图片到临时文件（使用HttpURLConnection，与参考项目一致）
            ...
            Log.d(TAG, GPS_DEBUG + "发送HTTP请求...");
            connection.connect();
            
            int responseCode = connection.getResponseCode();
            String contentType = connection.getContentType();
            int contentLength = connection.getContentLength();
            Log.d(TAG, GPS_DEBUG + "响应码: " + responseCode);
            Log.d(TAG, GPS_DEBUG + "Content-Type: " + contentType);
            Log.d(TAG, GPS_DEBUG + "Content-Length: " + contentLength);
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                while ((bytesRead = inputStream.read(buffer)) != -1) {
                    ...
                    if (totalBytes > 10 * 1024 * 1024) { // 10MB
                        Log.w(TAG, GPS_DEBUG + "图片文件过大，停止下载: " + totalBytes + " bytes");
                        break;
                    }
                }
                outputStream.flush();
                ...
                Log.d(TAG, GPS_DEBUG + "图片下载完成，大小: " + totalBytes + " bytes");
                
                // 从临时文件读取EXIF信息
                ExifInterface exif = new ExifInterface(tempFile.getAbsolutePath());
                
                // 打印所有可能的GPS相关EXIF属性
                Log.d(TAG, GPS_DEBUG + "EXIF信息:");
                Log.d(TAG, GPS_DEBUG + "  TAG_GPS_LATITUDE: " + exif.getAttribute(ExifInterface.TAG_GPS_LATITUDE));
                Log.d(TAG, GPS_DEBUG + "  TAG_GPS_LONGITUDE: " + exif.getAttribute(ExifInterface.TAG_GPS_LONGITUDE));
                Log.d(TAG, GPS_DEBUG + "  TAG_GPS_LATITUDE_REF: " + exif.getAttribute(ExifInterface.TAG_GPS_LATITUDE_REF));
                Log.d(TAG, GPS_DEBUG + "  TAG_GPS_LONGITUDE_REF: " + exif.getAttribute(ExifInterface.TAG_GPS_LONGITUDE_REF));
                Log.d(TAG, GPS_DEBUG + "  TAG_MAKE: " + exif.getAttribute(ExifInterface.TAG_MAKE));
                Log.d(TAG, GPS_DEBUG + "  TAG_MODEL: " + exif.getAttribute(ExifInterface.TAG_MODEL));
                Log.d(TAG, GPS_DEBUG + "  TAG_DATETIME: " + exif.getAttribute(ExifInterface.TAG_DATETIME));
                
                // 获取GPS坐标
                float[] latLong = new float[2];
                boolean hasLatLong = exif.getLatLong(latLong);
                Log.d(TAG, GPS_DEBUG + "EXIF GPS坐标: " + (hasLatLong ? latLong[0] + "," + latLong[1] : "null"));
                
                if (hasLatLong) {
                    double latitude = latLong[0];
                    double longitude = latLong[1];
                    Log.d(TAG, GPS_DEBUG + "✅ 成功提取GPS坐标: lat=" + latitude + ", lon=" + longitude);
                    return getLocationFromCoordinates(context, latitude, longitude);
                } else {
                    Log.d(TAG, GPS_DEBUG + "❌ 图片EXIF中没有GPS坐标信息");
                }
            } else {
                Log.w(TAG, GPS_DEBUG + "❌ 图片请求失败，响应码: " + responseCode);
            }
        } catch (Throwable e) {
            // 捕获所有异常，包括 RuntimeException 和 Error
            Log.e(TAG, GPS_DEBUG + "获取图片地点失败: " + e.getMessage(), e);
        } finally {
            // 清理资源
            try {
                if (inputStream != null) {
            ...
            if (tempFile != null && tempFile.exists()) {
                ...
                Log.d(TAG, GPS_DEBUG + "临时文件删除" + (deleted ? "成功" : "失败"));
            }
        }
        
        Log.d(TAG, GPS_DEBUG + "========== 图片GPS提取结束 ==========");
        return null;
    }
    ...
    public static String getLocationFromVideo(Context context, String videoUrl) {
        ...
        tasks.add(() -> {
            ...
            try {
                ...
                Log.d(TAG, GPS_DEBUG + "[任务1] METADATA_KEY_DURATION: " + duration);
                Log.d(TAG, GPS_DEBUG + "[任务1] METADATA_KEY_VIDEO_WIDTH: " + width);
                Log.d(TAG, GPS_DEBUG + "[任务1] METADATA_KEY_VIDEO_HEIGHT: " + height);
        ...
        // 任务4: 尝试下载完整视频到临时文件并提取（作为最后的手段）
        tasks.add(() -> {
            Log.d(TAG, GPS_DEBUG + "[任务4] 开始下载完整视频到临时文件");
            String location = extractGPSFromDownloadedVideo(context, videoUrl);
            if (location != null) {
                Log.d(TAG, GPS_DEBUG + "[任务4] ✅ 成功: 从下载的视频解析到位置: " + location);
                return location;
            } else {
                Log.d(TAG, GPS_DEBUG + "[任务4] 下载的视频中未找到GPS信息");
            }
            throw new Exception("下载视频提取未找到GPS信息");
        });
    ...
    private static String processVideoData(Context context, byte[] data) {
        ...
        // 方法1: 文本搜索（优先，因为在TV版中验证这比mp4parser更有效）
        ...
        // 方法2: 尝试使用mp4parser解析（作为备用）
        Log.d(TAG, GPS_DEBUG + "文本搜索失败，尝试使用mp4parser解析...");
        location = parseGPSWithMp4Parser(context, data);
        if (location != null) {
            return location;
        }
        
        Log.d(TAG, GPS_DEBUG + "❌ 所有方法均失败");
        return null;
    }
     ...
     * 使用mp4parser库解析GPS信息
     */
    private static String parseGPSWithMp4Parser(Context context, File videoFile) {
        IsoFile isoFile = null;
        
        try {
            // 使用mp4parser直接解析文件，避免内存溢出
            isoFile = new IsoFile(videoFile.getAbsolutePath());
            
            // 查找MovieBox
            MovieBox movieBox = isoFile.getMovieBox();
            if (movieBox == null) {
                Log.d(TAG, GPS_DEBUG + "mp4parser: 未找到MovieBox");
                return null;
            }
            ...
            for (Box box : boxes) {
                if (box instanceof UserDataBox) {
                    ...
                    Log.d(TAG, GPS_DEBUG + "mp4parser: 找到UserDataBox");
                    
                    // 查找GPS坐标Box (©xyz)
                    for (Box subBox : udta.getBoxes()) {
                        Log.d(TAG, GPS_DEBUG + "mp4parser: 子Box类型: " + subBox.getType());
                        
                        if (subBox instanceof AppleGPSCoordinatesBox) {
                            AppleGPSCoordinatesBox gpsBox = (AppleGPSCoordinatesBox) subBox;
                            String gpsValue = gpsBox.getValue();
                            Log.d(TAG, GPS_DEBUG + "mp4parser: 找到GPS Box, 值: " + gpsValue);
                            
                            if (gpsValue != null && !gpsValue.isEmpty()) {
                                String location = parseLocationString(context, gpsValue);
                                if (location != null) {
                                    return location;
                                }
                            }
                        ...
                        // 尝试其他可能包含GPS的Box
                        if ("©xyz".equals(subBox.getType())) {
                            Log.d(TAG, GPS_DEBUG + "mp4parser: 找到©xyz Box");
                            // 尝试读取Box内容
                        }
                    }
                }
            ...
            Log.d(TAG, GPS_DEBUG + "mp4parser: 未找到GPS信息");
            
        } catch (Exception e) {
            Log.e(TAG, GPS_DEBUG + "mp4parser解析失败: " + e.getMessage());
            e.printStackTrace();
        } finally {
            try {
                if (isoFile != null) {
    ...
    /**
     * 重载方法，支持byte[]输入（用于处理部分数据）
     */
    private static String parseGPSWithMp4Parser(Context context, byte[] data) {
        File tempFile = null;
        try {
            // 创建临时文件
            tempFile = File.createTempFile("mp4parser_", ".mp4", context.getCacheDir());
            java.io.FileOutputStream fos = new java.io.FileOutputStream(tempFile);
            fos.write(data);
            fos.close();
            
            return parseGPSWithMp4Parser(context, tempFile);
        } catch (Exception e) {
            Log.e(TAG, GPS_DEBUG + "mp4parser(byte[])解析失败: " + e.getMessage());
            return null;
        } finally {
            if (tempFile != null && tempFile.exists()) {
                tempFile.delete();
    ...
    private static String searchForGPSCoordinates(Context context, byte[] data) {
        try {
            ...
            String[] patterns = {
                ...
                // 新增：不带符号的坐标格式（默认为正）
                "\\d{2,3}\\.\\d{4,}\\s*,\\s*\\d{2,3}\\.\\d{4,}",  // 22.5430, 114.0578
                "\\d{2,3}\\.\\d{4,}\\s+\\d{2,3}\\.\\d{4,}",       // 22.5430 114.0578
                
                // 新增：更宽松的浮点数匹配
                "([+-]?\\d+\\.\\d+)[,\\s]+([+-]?\\d+\\.\\d+)",      // 任意两个浮点数
                
                // 新增：十六进制格式的坐标（某些设备可能使用）
    ...
    private static byte[] downloadVideoHeader(String videoUrl, int maxSize) {
        ...
        try {
            ...
            Log.d(TAG, GPS_DEBUG + "Content-Type: " + connection.getContentType());
     ...
     * 作为最后的手段，当其他方法都失败时使用
     */
    private static String extractGPSFromDownloadedVideo(Context context, String videoUrl) {
        File tempFile = null;
        Response response = null;
        InputStream inputStream = null;
        
        try {
            Log.d(TAG, GPS_DEBUG + "[任务4] 开始下载视频到临时文件");
            
            // 使用OkHttp下载视频（有更好的超时机制）
            ...
            Log.d(TAG, GPS_DEBUG + "[任务4] 视频请求响应码: " + responseCode);
            Log.d(TAG, GPS_DEBUG + "[任务4] Content-Type: " + response.header("Content-Type"));
            
            if (response.isSuccessful() && response.body() != null) {
                inputStream = response.body().byteStream();
                
                // 创建临时文件
                tempFile = File.createTempFile("video_gps_", ".tmp", context.getCacheDir());
                java.io.FileOutputStream outputStream = new java.io.FileOutputStream(tempFile);
                ...
                while ((bytesRead = inputStream.read(buffer)) != -1) {
                    ...
                    if (totalBytes > 50 * 1024 * 1024) { // 50MB
                        Log.w(TAG, GPS_DEBUG + "[任务4] 视频文件过大，停止下载: " + totalBytes + " bytes");
                        break;
                    }
                    
                    // 检查是否超时（总下载时间超过120秒则停止）
                    if (System.currentTimeMillis() - startTime > 120000) {
                        Log.w(TAG, GPS_DEBUG + "[任务4] 视频下载超时，停止下载");
                        break;
                    }
                }
                outputStream.flush();
                ...
                Log.d(TAG, GPS_DEBUG + "[任务4] 视频下载完成，大小: " + totalBytes + " bytes");
                
                // 使用MediaMetadataRetriever从本地文件提取元数据
                MediaMetadataRetriever retriever = new MediaMetadataRetriever();
                try {
                    retriever.setDataSource(tempFile.getAbsolutePath());
                    
                    String locationString = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_LOCATION);
                    Log.d(TAG, GPS_DEBUG + "[任务4] METADATA_KEY_LOCATION: " + locationString);
                    ...
                    // 如果标准方法失败，尝试mp4parser解析（优先使用，因为它可以处理大文件）
                    Log.d(TAG, GPS_DEBUG + "[任务4] 标准方法失败，尝试mp4parser解析");
                    String location = parseGPSWithMp4Parser(context, tempFile);
                    if (location != null) {
                        return location;
                    }

                    // 最后尝试文本搜索（只读取头尾部分，避免加载整个文件）
                    Log.d(TAG, GPS_DEBUG + "[任务4] mp4parser失败，尝试文本搜索");
                    // 仅读取前1MB
                    int headSize = 1024 * 1024;
                    byte[] headData = new byte[(int) Math.min(tempFile.length(), headSize)];
                    FileInputStream fis = new FileInputStream(tempFile);
                    fis.read(headData);
                    fis.close();
                    
                    location = searchForGPSCoordinates(context, headData);
                    if (location != null) {
                        return location;
                    }
            ...
            Log.e(TAG, GPS_DEBUG + "[任务4] 下载视频提取GPS失败: " + e.getMessage());
            e.printStackTrace();
        } finally {
            // 清理资源
            try {
            ...
            if (tempFile != null && tempFile.exists()) {
                boolean deleted = tempFile.delete();
                Log.d(TAG, GPS_DEBUG + "[任务4] 临时文件删除" + (deleted ? "成功" : "失败"));
            }
        }
